...
def _main():
    ...
    parser.add_argument("--compactness", type=int, default=1024,
        help="VAE compactness factor. Input samples are truncated to a multiple of this.")
    parser.add_argument("--input-dir", type=str, default="/data/iemanja",
        help="Original Iemanja dataset directory containing database.csv and data/")
    parser.add_argument("--output-dir", type=str, default="/data/iemanja_vae",
    ...
    if not os.path.isdir(input_data_dir):
        raise FileNotFoundError(f"Dataset data directory not found: {input_data_dir}")
    ...
    model = ml_utils.load_ts_model(args.model, device=device)
    ...
    for wav_path in tqdm.tqdm(wav_files):
        ...
        output_file = os.path.join(data_output_dir, filename)
        ...
        if file_id is not None and file_id in validation_ids:
            ...
            ml_audio.save_comparison(
                signals=[original_data, recon_data],
                labels=["Original", "Reconstrução"],
                fs=fs,
                output_base=os.path.join(comparison_dir, file_stem),
                plots=("psd", "mel"),
            )
